            ...
            <div   class="cotent_full">
              <div v-for="(item,index) of OrderList.data" :key="index" @click.stop="getText(item)">
                <div class="allorders-content-item">
                  ...
                  <div>
                    <span class="allorders-content-item-footer-part1-serviceDate">【线上】:<span   v-for="item of item.internet" :key="item">{{item}}、</span></span>
                  </div>
                  <div>
                    <span class="allorders-content-item-footer-part1-serviceDate">【线下】:<span   v-for="item of item.reality" :key="item">{{item}}、</span></span>
                  </div>
                  <div>
                    <span class="allorders-content-item-footer-part1-serviceDate">【服务日期】:{{item.providerTimeType}}</span>
                  </div>
                  <div>
                    <span class="allorders-content-item-footer-part1-serviceDate">【服务地点】:{{item.address.city  + '-'+  item.address.area}}</span>
                  </div>
                  <div>
                    <span class="allorders-content-item-footer-part1-serviceDate">-----------------------</span>
                  </div>
                  <div>
                    <span class="allorders-content-item-footer-part1-serviceDate">【联系方式】:
                      <span v-if="item.userInfo.username">微信</span>：{{item.userInfo.wechat}}、</span>
                      <span v-if="item.userInfo.username">QQ：{{item.userInfo.qq}}</span>
                  </div>
                  <div class="allorders-content-item-footer">
                    <p class="allorders-content-item-footer-part2"  >
                      <span class="allorders-content-item-footer-part2-btn" v-if="item.status == 22"  @click.stop="cancel(item)">取消订单</span>
                      <span class="allorders-content-item-footer-part2-btn allorders-content-item-footer-part2-btn-sure" v-if="item.status != 22">已结束</span>
                    </p>
                  </div>
                </div>
                <wrap :height='16'></wrap>
