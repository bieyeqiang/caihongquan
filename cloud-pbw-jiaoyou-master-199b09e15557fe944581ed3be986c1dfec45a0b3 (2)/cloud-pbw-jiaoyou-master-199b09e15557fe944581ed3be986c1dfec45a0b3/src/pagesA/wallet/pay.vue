<template>
  <view class="app column">
    ...
    <view class="cell row b-b" @click="checkType('wxpay')">
      <image class="icon" :src="'https://pbw-uploadfile.oss-cn-beijing.aliyuncs.com/image/icon/wxpay.png'"></image>
      <view class="column fill">
        <text class="tit">微信支付</text>
      </view>
      <u-icon v-if="curType === 'wxpay'" name="checkmark-circle-fill" color="#2979ff" size="40"></u-icon>
    </view>
    <!-- #ifndef MP-WEIXIN -->
    <view class="cell row b-b" @click="checkType('alipay')">
      <image class="icon" :src="'https://pbw-uploadfile.oss-cn-beijing.aliyuncs.com/image/icon/alipay.png'"></image>
      <view class="column fill">
        <text class="tit">支付宝</text>
      </view>
      <u-icon v-if="curType === 'alipay'" name="checkmark-circle-fill" color="#2979ff" size="40"></u-icon>
    </view>
    <!-- #endif -->
    <view class="cell row b-b" @click="checkType('balance')">
      <image class="icon" :src="'https://pbw-uploadfile.oss-cn-beijing.aliyuncs.com/image/icon/balance.png'"></image>
      <view class="column fill">
        <text class="tit">余额</text>
        <text class="tip">账户可用余额 ￥{{ userInfo.money | price }}</text>
      </view>
      <u-icon v-if="curType === 'balance'" name="checkmark-circle-fill" color="#2979ff" size="40"></u-icon>
    </view>

    <jw-button ref="confirmBtn" text="确认支付" marginTop="80rpx" @onConfirm="confirm"></jw-button>
    <!-- 支付成功面板 -->
    ...
    <jw-modal ref="mixModal" text="您还没有设置支付密码" confirmText="立即设置" @onConfirm="navTo('/pagesA/login/auth/payPassword')"></jw-modal>
  </view>
</template>
  ...
  methods: {
    ...
    successCallback() {
      switch (this.data.sourcePage) {
        case 'createOrder':
          uni.redirectTo({
            url: '/mall/order/detail?id=' + this.data.order_id
          })
          break
        case 'orderList':
        ...
        case 'vipOrder':
          ...
          uni.redirectTo({
            url: '/other/myhome/member/member'
          })
          break
      }
    },
