<template>
  <view class="app">
    ...
    <view class="input-wrap pwd-wrap row b-b">
      <text class="jw-icon icon-mima"></text>
      ...
      <text class="jw-icon icon-eye" :class="showPwd ? 'icon-yanjing': 'icon-biyan'" @click.stop.prevent="changePwdState"></text>
    </view>
    <view class="input-wrap row b-b">
      <text class="jw-icon icon-yanzhengma1"></text>
      <input class="input" v-model="code" type="number" maxlength="6" placeholder="请输入手机验证码" placeholder-style="color:#999;font-size:30rpx" />
      <jw-code templateCode="SMS_194050994" action="设置支付密码"></jw-code>
    </view>

    <jw-button ref="confirmBtn" text="提交" marginTop="80rpx" @onConfirm="confirm"></jw-button>
  </view>
</template>
