      ...
      <view class="tantan-slide-footer">
        <view class="tantan-slide-footer-btn" @click="footerBtnClick('dislike')">
          ...
          >
            <image
              ...
              src="https://pbw-uploadfile.oss-cn-beijing.aliyuncs.com/image/dgex-tantan/close_white.png"
            >
            </image>
          </view>
          <view class="tantan-slide-footer-icon">
            <image
              style="width: 50rpx; height: 50rpx"
              src="https://pbw-uploadfile.oss-cn-beijing.aliyuncs.com/image/dgex-tantan/close.png"
            ></image>
          </view>
        </view>
        <view class="tantan-slide-footer-btn" @click="footerBtnClick('love')">
          ...
          >
            <image
              style="width: 50rpx; height: 50rpx"
              src="https://pbw-uploadfile.oss-cn-beijing.aliyuncs.com/image/dgex-tantan/like_white.png"
            >
            </image>
          </view>
          <view class="tantan-slide-footer-icon">
            <image
              style="width: 50rpx; height: 50rpx"
              src="https://pbw-uploadfile.oss-cn-beijing.aliyuncs.com/image/dgex-tantan/like.png"
            ></image>
          </view>
        </view>
      </view>
...
export default {
  ...
  methods: {
    ...
    touchEnd (index) {
      ...
      if (this.list[index].x > 0 && this.list[index].x > this.winWidth / 2 - this.winWidth / 5) {
        this.touchEndNext(index)
      } else if (this.list[index].x < 0 && this.list[index].x < -this.winWidth / 2 + this.winWidth / 5) {
        this.touchEndNext(index)
      } else {
        this.list[index].x = 0
        this.list[index].y = 0
    ...
    async footerBtnClick (type) {
      ...
      let w = 0
      if (type === 'love') {
		console.log(this.list[this.currentIndex].uid,'111');
		const data = await this.$request('like', 'addLike', {
		  likeId: this.list[this.currentIndex].uid,
		  state: 0 // 0 1 
		})
		if(data){
		  w = this.winWidth * 1.5
...
.tantan-slide-box-item {
  ...
  transition: 300ms;
  color: #fff;
  box-shadow: 5rpx 8rpx 15rpx 4rpx #999999;
  border-radius: 40rpx;
}
