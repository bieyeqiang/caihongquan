...
export default {
  ...
  methods: {
    ...
    navToDetai (item) {
      this.navTo('/pagesA/public/article?param=' + JSON.stringify({
        module: 'notice',
        operation: 'getDetail',
        data: {
          id: item._id
  ...
  async onLoad () {
    ...
    console.log(res)
     ...
     let num = 0
    setInterval(() => {
      console.log(num + '-' + res.data[num].username + ':' + res.data[num]._id)
      console.log(res.data[num]._id,'ID')
        console.log(rand(7),'随机')
        // const user = await this.$request("user", "updateUserUid", {
        //   id: i._id,
        //   uid: rand(7)
        // });
      num++
    ...
    //   limit: 300,
    // });
    // console.log(res, '查到动态')
    // for (let item of res.data) {
    // for (let list of this.RECORDS) {
    //   if (item.userInfo.uid == list.userCode) {
    //     console.log(item.userInfo.uid, '匹配到用户')
    //     console.log(item._id, 'id是')
    //     console.log(list.dynamicLat, list.dynamicLng, '匹配到坐标')
    //     try {
    //       await this.$request("nearby", "editNearby", {
    //         id: item._id,
    //         longitude: Number(list.dynamicLng),
    //         latitude: Number(list.dynamicLat),
    //       }).then((res) => {
    //         console.log(res)
    //       });
    //     } catch (error) {
