      ...
      >
        <view v-if="tabIndex.index == 0">
          ...
          <view class="news-reply">
            <text>{{ item.userInfo.nickname }}：</text>
            <text v-if="item.reply">{{ item.reply.comment }}</text>
          </view>
        </view>
        <view v-if="tabIndex.index == 1">
          <view class="people">
            <image :src="item.reply.userInfo.avatar"></image>
            <view class="cont">
              <view class="name">
                <text>{{ item.reply.userInfo.nickname }}</text>
                <!-- <image src="https://pbw-uploadfile.oss-cn-beijing.aliyuncs.com/image/travel/news/dengji.png"></image> -->
              </view>
              <text class="time">{{ item.add_time | getTimeText }}</text>
            </view>
      ...
      uni.showActionSheet({
        ...
        success: res => {
          if (res.tapIndex == 0) {
            ...
            this.tabIndex.text = '@我的评论'
          } else {
            this.tabIndex.index = 1
            this.tabIndex.text = '我@Ta的'
          }
          this.getList()
        }
      })
