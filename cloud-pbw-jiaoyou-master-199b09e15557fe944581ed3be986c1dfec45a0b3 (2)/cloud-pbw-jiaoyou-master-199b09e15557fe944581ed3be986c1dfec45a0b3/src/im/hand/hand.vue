<template>
  <view>
    <view class="tabr">
      <view :class="{ on: typeClass == 'luck' }" @tap="switchType('luck')"
        >拼手气红包</view
      ><view :class="{ on: typeClass == 'normal' }" @tap="switchType('normal')"
        >普通红包</view
      >
    ...
    <view class="content" :class="typeClass">
      <view class="luck">
        <view class="row">
          ...
          <view class="input">
            <input type="number" v-model="number" placeholder="输入红包个数" />
            个
          </view>
        </view>
        <view class="row">
          ...
          <view class="input">
            <input type="number" v-model="luckMoney" placeholder="输入金额" />
            元
          </view>
        </view>
        <view class="tis">
...
export default {
  ...
  methods: {
    ...
    hand (type) {
      ...
      if (
        redEnvelopeData.number != Math.abs(parseInt(redEnvelopeData.number))
      ) {
      ...
      page.$vm.setRedEnvelope(redEnvelopeData)
      // 传完参 用后退即可
      uni.navigateBack({
        delta: 1
      })
