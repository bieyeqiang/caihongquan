<template>
	...
	>
		...
		<text v-if="icon" class="jw-icon" :class="icon" :style="{fontSize: iconSize + 'rpx'}"></text>
		<text class="jw-text">{{ text }}</text>
	</view>
</template>
...
<style scoped lang='scss'>
	.jw-btn-content{
		...
		&:after{
			...
			width: 85%;
			height: 85%;
			background: linear-gradient(131deg, #6665f6 0%, #6665f6 100%);
			border-radius: 100rpx;
			opacity: 0.4;
			filter:blur(10rpx);
		}
