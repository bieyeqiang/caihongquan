 ...
 * @LastEditTime: 2022-11-29 11:19:23
 * @Description:  用户匹配卡片
-->
<template>
  <view class="card">
    <view class="title">推荐的Ta</view>
        ...
        for (let item of res.data) {
          if (item.avatar) {
            ...
            dataGroup.push({
              image: src,
              sex: item.gender,
              address: '(' + vm.timeLog(item.last_login_date ? item.last_login_date : item.register_date) + ' 附近推荐)',
              title: item.nickname,
