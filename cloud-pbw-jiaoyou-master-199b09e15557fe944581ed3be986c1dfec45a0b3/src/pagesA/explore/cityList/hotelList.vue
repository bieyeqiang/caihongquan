  ...
  <view>
    ...
    <view class="card">
      <view class="head u-flex">
        <view class="title">附近推荐酒店</view>
        <view class="sub">旅行酒店品质推荐</view>
      </view>
      <!-- 商品盒子 -->
      <mescroll-uni top="150" @down="downCallback" :up="upOption" @up="upCallback" @emptyclick="emptyClick" @init="mescrollInit">
        <view class="goods-box u-flex" v-for="(item, index) of hotelData" :key="index" @click="navTo('/pagesA/explore/travelDetail?type=hotel&keywords=5A景区&id=' + item.id)" v-if="item.photos[0]">
          ...
          <view class="content u-flex-col">
            ...
            <view class="price u-flex" v-if="item.biz_ext && item.biz_ext.lowest_price && item.biz_ext.lowest_price.length>0">
              <text>￥</text>
