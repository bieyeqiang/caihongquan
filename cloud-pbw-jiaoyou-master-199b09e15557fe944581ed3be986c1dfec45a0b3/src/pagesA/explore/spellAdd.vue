  ...
  <view class="issueDyn-wrapper">
    <view class="allorders-content">
      <view>
        <view class="issueDyn-swiper-item">
          <view class="issueDyn-swiper-item-date">
            <picker class="picker-item" mode="date" @change="birthBtn" :start="starTime" :value="starTime">
              <text class="issueDyn-swiper-item-date-start">{{
                birthVal
              }}</text>
            </picker>
            <text class="issueDyn-swiper-item-date-icon"></text>
            <picker class="picker-item" mode="date" @change="birthBtn_end" :start="starTime" :value="starTime">
              <text class="issueDyn-swiper-item-date-end">{{
                birthVal_end
              }}</text>
            </picker>
          </view>
          <wrap :height="16"></wrap>
          <textarea v-model="nearbyVal" class="issueDyn-swiper-itemTxt" maxlength="500" style="min-height: 150px" auto-height placeholder="说点你的拼单要求..." />
          <view class="topicBg"> </view>
          <view class="xianbai">添加你的照片，更利于拼单成功~</view>
          ...
          <view class="issueDyn-swiper-itemLocation" @click="showModel">
            <text class="issueDyn-swiper-itemLocation-label iconfont icon-labelb"></text><text class="iconfont">&#xe61c;</text>标签：<text v-if="selectedLabel != ''" style="
                color: #eb5248;
          ...
          <view class="issueDyn-swiper-item-fun" @click="Location">
            <view class="issueDyn-swiper-itemLocation">
              <text class="issueDyn-swiper-itemLocation-label iconfont icon-dingwei"></text><text class="iconfont">&#xe738;</text>位置：{{ LocationText }}
            </view>
          </view>
          <wrap :height="2"></wrap>
    ...
    <view class="issueDyn-model" v-bind:style="[{ top: modelTop }]" @click="hiddenModel">
      <view class="issueDyn-model-content" @click.stop="">
        ...
        <!--<textarea @input='markTxt' class='issueDyn-model-content-txt' maxlength='20' placeholder="自定义标签" placeholder-style="color:#888;" />-->
        <!--<p class='issueDyn-model-content-txtLabel'>{{txtLength}}/20</p>-->
        <view class="issueDyn-model-content-labelBox">
          <p class="issueDyn-model-content-txtLabel-title">快捷标签：</p>
          <text class="issueDyn-model-content-txtLabel-icon" v-for="(item, itemIndex) in labelList" :key="itemIndex" @click="chooseLabel(item)" v-bind:class="[
              selectedLabel === item
...
export default {
  ...
  methods: {
      ...
      uni.chooseLocation({
        ...
        fail: function (res) {
          ...
          uni.showModal({
            title: "获取坐标失败",
            content:
              "您拒绝了授权获取坐标，无法为您定位，请在 [设置] 里打开坐标定位",
            success: function (res) {
              if (res.confirm) {
                //console.log("用户点击确定");
              } else if (res.cancel) {
    ...
    async addText (index) {
      ...
      // 旅行
      if (index == 2) {
        ////console.log(new Date(vm.birthVal + ' 00:00:00').getTime(),"2222")
        if (vm.birthVal == "选择出发日期") {
          uni.showToast({
            title: "请选择出发时间",
            icon: "none",
            duration: 3000,
        ...
        if (vm.nearbyVal == "") {
          uni.showToast({
            title: "请填写内容描述，最好在配张美美的照片^_^",
            icon: "none",
            duration: 3000,
          });
          return;
        }
        //console.log(new Date(vm.birthVal + " 00:00:00").getTime() / 1000);
        //console.log(new Date(vm.birthVal_end + " 00:00:00").getTime() / 1000);
        const res = await this.$request("nearby", "addNearby", {
          dynamicType: 2,
          spellType: 1,
          spellId: this.option.id,
          dynamicContext: vm.nearbyVal,
          dynamicImg: vm.SrcList,
          longitude: vm.longitude,
          latitude: vm.latitude,
          positionDesc: vm.LocationText,
          dynamicStart: new Date(vm.birthVal + " 00:00:00").getTime() / 1000,
          dynamicEnd: new Date(vm.birthVal_end + " 00:00:00").getTime() / 1000,
          dynamicLabel: vm.selectedLabel,
          topicTitle: vm.topicList,
        });
        if (res.status == 1) {
          uni.navigateBack({
            delta: 1,
  ...
  onLoad: async function (option) {
    ...
    //console.log(this.starTime, "9999999999999");
    // 获取定位
    uni.getLocation({
      type: "gcj02",
      success: (res) => {
  ...
  computed: {
    swiperHeight () {
      let relHeight =
        this.curIndex === 0 ? 600 : this.curIndex === 1 ? 920 : 670;
      return relHeight;
    },
    swiperLeft () {
      let relLeft =
        this.curIndex === 0 ? 0 : this.curIndex === 1 ? -750 : -1500;
      return relLeft;
    },
    modelTop () {
